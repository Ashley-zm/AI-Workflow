...
**ai-workflow** is a visual node-based AI workflow editor built with Vue 3, TypeScript, and Vue Flow. Users drag-and-drop nodes from a sidebar onto a canvas, configure LLM models and prompts via a properties panel, and connect nodes with edges to create AI processing pipelines.
...
- **Right Panel** (`PropertiesPanel.vue`): Reactive editor for selected node properties with slide-fade animations
...
- **Key mutation**: `setSelectedNode()` toggles the properties panel based on selection
...
- **Missing `data.config` initialization**: Always ensure config exists before accessing properties (done in onNodeClick + onDrop)
