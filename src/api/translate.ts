...
const BAIDU_TRANSLATE_PATH = '/baidu/api/trans/vip/translate'
...
const pickEnv = (...keys: string[]) => {
  for (const key of keys) {
    const value = normalizeEnv((import.meta.env as Record<string, string | undefined>)[key])
    if (value) return value
  }
  return ''
}

const BAIDU_APP_ID = pickEnv(
  'VITE_BAIDU_APP_ID',
  'VITE_BAIDU_APPID',
  'VITE_BAIDU_TRANSLATE_APP_ID',
  'VITE_BAIDU_TRANSLATE_APPID',
)
const BAIDU_APP_KEY = pickEnv(
  'VITE_BAIDU_APP_KEY',
  'VITE_BAIDU_APPKEY',
  'VITE_BAIDU_KEY',
  'VITE_BAIDU_SECRET_KEY',
  'VITE_BAIDU_TRANSLATE_APP_KEY',
)
export const hasBaiduTranslateConfig = Boolean(BAIDU_APP_ID && BAIDU_APP_KEY)

const rotateLeft = (value: number, shiftBits: number) =>
...
const toWordArray = (input: string) => {
  ...
  const wordArray = new Array<number>(numberOfWords - 1).fill(0)
...
const wordToHex = (value: number) => {
  ...
  for (let index = 0; index <= 3; index += 1) {
    ...
    const hex = `0${byteValue.toString(16)}`
    output += hex.slice(-2)
  }
  return output
}
...
const assertTranslateConfig = () => {
  if (!hasBaiduTranslateConfig) {
    throw new Error('Missing VITE_BAIDU_APP_ID or VITE_BAIDU_APP_KEY')
  }
}

export const translateZhToEn = async (text: string) => {
  const sourceText = text.trim()
  if (!sourceText) return ''

  assertTranslateConfig()

  const salt = `${Date.now()}${Math.floor(Math.random() * 10000)}`
  const sign = md5(`${BAIDU_APP_ID}${sourceText}${salt}${BAIDU_APP_KEY}`)
  ...
  const response = await fetch(`${BAIDU_TRANSLATE_PATH}?${params.toString()}`)
  const result = (await response.json()) as BaiduTranslateResponse
