      ...
      <div class="rounded-lg border border-slate-200 bg-white p-4 shadow-sm space-y-4">
        <div class="grid grid-cols-2 gap-4">
          <div>
            ...
            <div class="flex items-center gap-2">
              <el-color-picker v-model="polygonColor" show-alpha @change="updateConfig" />
              <span class="text-xs text-slate-500">{{ polygonColor }}</span>
            </div>
          </div>
          <div>
        ...
        <div class="flex items-center justify-between">
          ...
          <el-switch v-model="fillPolygon" @change="updateConfig" />
        </div>

        <div v-if="fillPolygon" class="space-y-4 pt-2 border-t border-slate-200">
          <div class="grid grid-cols-2 gap-4">
            <div>
              <label class="block text-xs font-medium text-slate-700 mb-2">填充颜色</label>
              <div class="flex items-center gap-2">
                <el-color-picker v-model="fillColor" show-alpha @change="updateConfig" />
                <span class="text-xs text-slate-500">{{ fillColor }}</span>
              </div>
            </div>
            <div>
...
const polygonColor = ref('#FF0000')
const polygonWidth = ref(2)
const polygonOpacity = ref(1)
const fillPolygon = ref(false)
...
const availablePredictionNodes = computed(() => {
  return store.nodes
    .filter((node) => {
      const hasPredictionProperty = modelTypes.some((prop: any) => node.type === prop)
...
const updateConfig = () => {
  let newConfig: Record<string, any> = {
    ...
    polygonWidth: polygonWidth.value,
    polygonOpacity: polygonOpacity.value,
    fillPolygon: fillPolygon.value,
...
const initializeData = () => {
  if (props.modelValue.length > 0) {
    ...
    polygonColor.value = props.modelValue[0].polygonColor || '#FF0000'
    polygonWidth.value = props.modelValue[0].polygonWidth || 2
    polygonOpacity.value = props.modelValue[0].polygonOpacity || 1
    fillPolygon.value = props.modelValue[0].fillPolygon || false
    fillColor.value = props.modelValue[0].fillColor || '#FF0000'
    fillOpacity.value = props.modelValue[0].fillOpacity || 0.3
...
const clearData = () => {
  selectedImagePath.value = ''
  selectedPredictionPath.value = ''
  polygonColor.value = '#FF0000'
  polygonWidth.value = 2
  polygonOpacity.value = 1
  fillPolygon.value = false
